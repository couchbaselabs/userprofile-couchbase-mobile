...
:page-aliases: tutorials:userprofile:background-fetch
...
- Next, we will download the latest version of Couchbase Lite 2.x framework. The Couchbase Lite iOS framework is link:https://developer.couchbase.com/documentation/mobile/current/couchbase-lite/swift.html[distributed] via Cocoapods, Carthage or you can download the pre-built framework. In our example, we will be downloading the pre-built version of the framework. For this, type the following in the command terminal
+
[source,bash] 
----
...
[source,swift]
----
include::{examplesdir}/UserProfileDemo/AppDelegate.swift[tags=setMinBackgroundFetchInterval,indent=0]
----
...
[source,swift]
----
include::{examplesdir}/UserProfileDemo/AppDelegate.swift[tags=backgroundFetchHandler,indent=0]
----
...
Setting up the app for one-shot replication is very similar to continuous replication that was discussed in the link:https://docs.couchbase.com/tutorials/userprofile-couchbase-mobile/sync/userprofile/userprofile_sync.html#starting-replication[Sync tutorial].

* Open the *DatabaseManager.swift* file and locate the `startOneShotPullReplicationForCurrentUser()` function. 
+
[source,swift]
----
include::{examplesdir}/UserProfileDemo/model/DatabaseManager.swift[tags=startOneShotPullReplicationForCurrentUser,indent=0]]
----
* Let's examine the replicator configuration
+
[source,swift]
----
include::{examplesdir}/UserProfileDemo/model/DatabaseManager.swift[tags=oneshotreplicationconfig,indent=0]
----
...
* Open the command terminal and issue the following `curl` command to get the user profile document on the Sync Gateway via link:[GET Document REST API] 
+
[source,bash]
----
curl -X GET \
...
----
* In the command terminal, issue the following command to update the user profile document on the Sync Gateway via link:[PUT Document REST API]
+
[source,bash]
----
curl -X PUT \
...
+
image::relaunch_app.png[App Launched into foreground]
