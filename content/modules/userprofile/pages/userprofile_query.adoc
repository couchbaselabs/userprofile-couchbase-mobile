...
== Introduction
Couchbase Lite 2.x brings powerful querying and Full-Text-Search(FTS) capabilties to the edge. The new query interface is based on link:https://www.couchbase.com/products/n1ql[N1QL], Couchbase’s declarative query language that extends link:https://www.sqlite.org/index.html[SQL] for JSON. If you are familiar with SQL, you will feel right at home with the semantics of the new API.  The query API is designed using the link:https://en.wikipedia.org/wiki/Fluent_interface[Fluent API Design Pattern], and it uses method cascading to read to like a Domain Specific Language (DSL). This makes the interface very intuitive and easy to understand.
...
* Demonstrate how you can bundle, load and use a  **_prebuilt_** instance of Couchbase Lite 2.x 
...
* If you are unfamiliar with the basics of Couchbase Lite, it is recommended that you walk through the link:https://developer.couchbase.com/documentation/mobile/2.0/userprofile_basic.html[Fundamentals Tutorial] on using Couchbase Lite 2.x as a standalone database
...
- Next, we will download the Couchbase Lite 2.8 framework. The Couchbase Lite iOS framework is link:https://developer.couchbase.com/documentation/mobile/current/couchbase-lite/swift.html[distributed] via Cocoapods, Carthage or you can download the pre-built framework. In our example, we will be downloading the pre-built version of the framework. For this, do the following
+
[source,bash] 
----
