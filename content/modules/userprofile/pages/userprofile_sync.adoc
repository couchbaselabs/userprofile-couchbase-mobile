...
* Allows users to log in and create or update his/her user profile information. The user profile view is _automatically_ updated everytime the profile information changes in the underlying database
...
* Open the `*sync-gateway-config-userprofile-walrus.json*` file using any text editor of your choice. The `sync-gateway-config-userprofile-walrus.json` is located in the app bundle at `/path/to/UserProfileDemo/content/modules/userprofile/examples`. Locate the following settings.
