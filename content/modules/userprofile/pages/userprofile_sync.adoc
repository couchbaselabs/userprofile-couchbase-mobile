...
:config-file-legacy--url: {git-sync--url}{config-filename}-2-x.json
:config-file--url: {git-sync--url}{config-filename}-3-x-legacy.json
...
.{alt-text}
image::userprofile-app-overview.gif[{alt-text}]
...
Start with Sync Gateway {target-version-full}, the sync{nbsp}gateway needs to be bootstrapped with a startup configuration file.
Alternatively, you can opt to run in legacy-mode and use the Pre-{target-version-full} configuration.

When starting Sync Gateway {target-version-full}, we need only provide the bootstrap configuration information in the config file -- see: {sync-gateway-config-overview--xref}.
We could then provision any Sync Gateway database configuration using the REST endpoints
...
The configuration files corresponding to this sample application are shown in xref:tbl-config-files[].
...
+
You should see configuration files for the latest major version and the previous major version.
...
+
--
Configuring and running Sync Gateway {target-version} in Docker using the configuration in `sync-gateway-config-userprofile-demo-3-x-legacy.json`.
...
We will use the Sync functions {sync-function-api-require-user--xref} API to verify that the `email` property specified in the Document matches the Id of the user making the request -- see <<ex-auth>>.

The Id of the user making the request is specified in the `Authorization` header.
We will use _Basic Authentication_ in our application -- at this stage our function will look like that shown here <<ex-auth>>.
...
<.> If this a new document, then verify that the `Id` of the Document is of the required format (i.e. _"user::<email>"_).
