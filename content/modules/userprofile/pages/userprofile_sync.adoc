...
This tutorial will walk through a simple swift app that will demonstrate how to 
...
* Set up your Sync Gateway to enforce data routing, access control and authorization
* Set up Live Queries within your Couchbase Lite clients so they are asyncronously notified of changes
...
* On one simulator, enter values in the user and address fields (you can enter other fields as well)
...
----
curl -X PUT \
  'http://localhost:4985/userprofile/user::demo@example.com?rev=2-3a76cfa911e2c54d1e82b29dbffc7f4e5a9bc265' \
  -H 'Accept: application/json' \
  -H 'Cache-Control: no-cache' \
  -H 'Content-Type: application/json' \
  -d '{
...
* link:https://blog.couchbase.com/querying-array-collections-couchbase-mobile/[Sync Function API]
