...
* Then we create a local Couchbase Lite database for the user. If a database already exists for the user, the existing version is returned. 
[source,swift]
----
...
=== Reading a Document
Once the user logs in, the user is taken to the "Your Profile" screen. A request is made to load  <<The "User Profile" Document>>  for the user. The very first time that the user logs in, there would be no _user profile_ document for the user.
...
[source,swift]
----
include::{codedir}/model/DatabaseManager.swift[tags=fetchRecordForCurrentUser]
----
